export default {
  ...
  methods: {
    validateAll() {
      ...
      if (!this.form.actividad_id) {
        this.errors.actividad_name = 'La actividad es obligatoria.';
      }
    ...
    onActividadSearchInput() {
      ...
      this.searchTimeout = setTimeout(() => this.fetchActividades(1), 300);
    },
    async fetchActividades(page = 1) {
      ...
      try {
        ...
        params.append('limit', this.actividadPerPage);
        ...
        this.actividadResults = json.data || [];
        if (json.pagination) {
          this.actividadPage = json.pagination.page;
          this.actividadTotalPages = json.pagination.last_page;
          this.actividadTotal = json.pagination.total;
        }
      } catch (e) {
        this.actividadResults = [];
      } finally {
    ...
    goActividadPage(page) {
      if (page < 1 || page > this.actividadTotalPages) return;
      this.fetchActividades(page);
    },
    selectActividad(act) {
      this.form.actividad_id = act.id;
      this.form.actividad_name = `${act.codigo_producto} - ${act.producto}`;
    ...
    async loadLicitacionData() {
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (data.actividad) {
            const codigoProducto = data.actividad.codigo_producto || '';
            const producto = data.actividad.producto || '';
            const display = `${codigoProducto} - ${producto}`.trim();
            this.form.actividad_name = display !== '-' ? display : '';
          } else {
  ...
  template: `
    <div class="container-fluid mt-4">
      ...
      <div v-show="activeTab === 'general'">
        ...
        <div class="mb-3">
          <label for="actividadSelectedInput" class="form-label">Actividad <span class="text-danger">*</span></label>
          <div class="d-flex gap-2">
            <input id="actividadSelectedInput" :class="['form-control', errors.actividad_name ? 'is-invalid' : '']" readonly :value="form.actividad_name || 'Ninguna seleccionada'">
      ...
      <div v-if="showActividadModal" class="modal-backdrop d-flex align-items-center justify-content-center" style="position:fixed;inset:0;z-index:1050;">
        <div class="bg-white rounded shadow p-3" style="width:1300px;max-width:98%;max-height:90vh;overflow:auto;">
          ...
          <div class="mb-2">
            <input id="actividadSearchInput" v-model="actividadQuery" @input="onActividadSearchInput" class="form-control" placeholder="Buscar por código o descripción">
          </div>
          <div class="mb-2">
              ...
              <table class="table table-sm table-hover align-middle mb-0">
                <thead class="table-light">
                  <tr>
                    ...
                    <th style="width:120px;">Cod. Segmento</th>
                    <th>Segmento</th>
                    <th style="width:120px;">Cod. Familia</th>
                    <th>Familia</th>
                    <th style="width:120px;">Cod. Clase</th>
                    <th>Clase</th>
                ...
                <tbody>
                  <tr v-for="act in actividadResults" :key="act.id">
                    <td>{{ act.id }}</td>
                    <td>{{ act.codigo_segmento }}</td>
                    <td>{{ act.segmento }}</td>
                    <td>{{ act.codigo_familia }}</td>
                    <td>{{ act.familia }}</td>
                    <td>{{ act.codigo_clase }}</td>
                    <td>{{ act.clase }}</td>
                    <td>{{ act.codigo_producto }}</td>
                    <td>{{ act.producto }}</td>
                    <td class="text-end">
                      <button type="button" class="btn btn-sm btn-outline-primary" @click="selectActividad(act)">Seleccionar</button>
                    </td>
                  </tr>
                  <tr v-if="!searchLoading && actividadResults.length===0">
                    <td colspan="10" class="text-muted text-center py-3">Sin resultados</td>
                  </tr>
                </tbody>
              </table>
            </div>
          ...
          <div class="d-flex justify-content-between align-items-center mt-2">
            <small class="text-muted">{{ actividadTotal }} resultados</small>
            <div class="btn-group" role="group">
              <button type="button" class="btn btn-sm btn-outline-secondary" :disabled="actividadPage <= 1" @click="goActividadPage(actividadPage - 1)">Anterior</button>
              <button type="button" class="btn btn-sm btn-outline-secondary" :disabled="actividadPage >= actividadTotalPages" @click="goActividadPage(actividadPage + 1)">Siguiente</button>
            </div>
          </div>
        </div>
      </div>
